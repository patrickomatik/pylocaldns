...
""")
    ...
    def test_scan_results_with_ports(self, mock_scan_network):
        ...
        with patch.object(self.hosts_file, '_add_preallocated_ip'):
            ...
            # First make sure we can access the scan_results attribute
            if not hasattr(WebUIHandler, 'scan_results'):
                WebUIHandler.scan_results = {}
            
            # Then set the scan results that would be set by the scan thread
            WebUIHandler.scan_results = {
                '192.168.1.100': {
                    'mac': '11:22:33:44:55:66',
                    'status': 'Added',
